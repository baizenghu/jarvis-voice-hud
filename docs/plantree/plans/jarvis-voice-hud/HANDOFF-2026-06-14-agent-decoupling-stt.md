...
- **🆕 phase5 部署态(机器重启需重做)**:**中心语音栈一键起 = `bash hud-app/start_voice_center.sh`**(幂等:CosyVoice 8003 在跑则跳过、voice_svc 8011 按端口重起;从 `~/.jarvis-secrets` 读 `JARVIS_GATEWAY_TOKEN`)。中心 `~/.jarvis-secrets`(600,不入库)存 `JARVIS_GATEWAY_TOKEN=<tok>`,**须与 0.3 HUD 的 `JARVIS_TOKEN` 一致**。0.3 `~/.jarvis-secrets` 末尾有 `JARVIS_USE_HTTP_VOICE=1`/`JARVIS_VOICE_URL=http://10.8.0.2:8011`/`JARVIS_TOKEN=<tok>`(同一 token)。**HUD 喊话走中心 voice_svc,不再经网关 WS STT。** whisper_api:8010 仍在跑(Windows hermes 用 + 回退兜底)。
